...
def create_service(build_name, version):
    ...
    try:
        ...
        if os.path.exists("app_configs/google_token.json"):
            creds = Credentials.from_authorized_user_file("app_configs/google_token.json", scopes)
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file("app_configs/client_secret.json", scopes)
                creds = flow.run_local_server(port=0)

            # Save the credentials for the next run
            with open(f"app_configs/google_token.json", 'w') as token:
